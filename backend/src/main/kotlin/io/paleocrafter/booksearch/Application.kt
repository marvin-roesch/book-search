...
fun Application.main() {
    ...
    transaction {
        SchemaUtils.create(Books, TableOfContentEntries)
    }
    ...
    routing {
        route("api") {
            put("book") {
                ...
                val buffer = ByteArrayOutputStream().also { output ->
                    output.use { file.streamProvider().use { it.copyTo(output) } }
                }.toByteArray()
            ...
            put("/book/{id}/table-of-contents") {
                ...
                val classes = transaction(db) {
                    ...
                    linearized.flatMapTo(mutableSetOf()) {
                        it.second.extractCSSClasses()
                    }
                } ?: return@put call.respond(
                    HttpStatusCode.NotFound,
                    mapOf("message" to "Book with ID '$id' does not exist")
...
fun TOCReference.extractCSSClasses(): Set<String> {
    val content = Jsoup.parse(String(this.resource.data)).body()

    return content.allElements.flatMapTo(mutableSetOf()) { it.classNames() }
}
